...
class CustomNodeOperator(bpy.types.Operator):
    """Tooltip"""
    bl_idname = "node.custom_operator"
    bl_label = "Custom Node Operator"
...
def menu_func(self, context):
    self.layout.operator(CustomNodeOperator.bl_idname, text=CustomNodeOperator.bl_label)


# Register and add to the "Node" menu (required to also use F3 search "Simple Node Operator" for quick access).
def register():
    bpy.utils.register_class(CustomNodeOperator)
    bpy.types.NODE_MT_node.append(menu_func)


def unregister():
    bpy.utils.unregister_class(CustomNodeOperator)
